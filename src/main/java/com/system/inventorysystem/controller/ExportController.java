...
import com.system.inventorysystem.service.*;
...
public class ExportController {
    ...
    public String save(@RequestParam(required = false) Long customerId,
        ...
        try {
            ...
            if (customerId != null) {
                // Load đầy đủ Customer từ DB — Strategy cần đọc customerType
            ...
            for (int i = 0; i < productIds.size(); i++) {
                ...
                Product product = Product.builder().id(productIds.get(i)).build();
                ...
                if (expiryDates != null && i < expiryDates.size()) {
                    String expiryDate = expiryDates.get(i);
                    if (expiryDate != null && !expiryDate.isBlank()) {
                        d.setExpiryDate(LocalDate.parse(expiryDate));
                    }
                }
                if (batchNumbers != null && i < batchNumbers.size()) {
                    String batchNumber = batchNumbers.get(i);
                    if (batchNumber != null && !batchNumber.isBlank()) {
                        d.setBatchNumber(batchNumber);
                    }
                }
    ...
    public String complete(@PathVariable Long id, RedirectAttributes ra) {
        try {
            exportService.complete(id);
            ra.addFlashAttribute("success", "Xác nhận xuất kho thành công!");
        } catch (Exception e) {
            ra.addFlashAttribute("error", e.getMessage());
        }
    ...
    public String cancel(@PathVariable Long id, RedirectAttributes ra) {
        try {
            ...
            ra.addFlashAttribute("success", "Hủy phiếu xuất thành công!");
        } catch (Exception e) {
            ra.addFlashAttribute("error", e.getMessage());
        }
        return "redirect:/exports";
    ...
    public ResponseEntity<?> getProductInfo(@PathVariable Long id) {
        return productService.findById(id).map(p -> {
            ...
            d.put("unit", p.getUnitName());
            d.put("sellPrice", p.getSellPrice());
