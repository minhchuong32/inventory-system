...
public class DataSeederConfig {
    ...
    public CommandLineRunner seedDatabase(JdbcTemplate jdbcTemplate, PasswordEncoder passwordEncoder) {
        return args -> {
            ...
            // Bước này rất quan trọng để tránh việc mỗi lần container restart lại bị lỗi báo trùng lặp (Duplicate Key).
            ...
            if (count != null && count == 0) {
                ...
                String insertUserSql = """
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, GETDATE(), 'system', GETDATE(), 'system', 0, NULL, NULL)
                """;

                // Thuật toán passwordEncoder.encode() sẽ tự động băm mật khẩu "123456" thành mã BCrypt
                jdbcTemplate.update(insertUserSql, "admin", passwordEncoder.encode("123456"), "Quản trị viên", "admin@ims.vn", "0901000001", "ADMIN", 1);
                jdbcTemplate.update(insertUserSql, "manager", passwordEncoder.encode("123456"), "Nguyễn Quản Lý", "manager@ims.vn", "0901000002", "MANAGER", 1);
                jdbcTemplate.update(insertUserSql, "staff1", passwordEncoder.encode("123456"), "Trần Nhân Viên", "staff1@ims.vn", "0901000003", "STAFF", 1);
                ...
                // ======================================================
                // Sử dụng Text Block (""") của Java để dán trực tiếp câu lệnh SQL mà không cần nối chuỗi
